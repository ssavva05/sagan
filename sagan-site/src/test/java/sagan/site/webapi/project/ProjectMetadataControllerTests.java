...
public class ProjectMetadataControllerTests {
	...
	@Test
	public void showSpringBootProject() throws Exception {
		...
		this.mvc.perform(get("/api/projects/spring-boot").accept(MediaTypes.HAL_JSON))
				.andExpect(status().isOk())
				.andDo(document("project-show", responseFields(projectPayload()), links(projectLinks())));
	}
	...
	FieldDescriptor[] projectPayload() {
		return new FieldDescriptor[] {
				...
				fieldWithPath("status").type(JsonFieldType.STRING).description("Support status of the project"),
	...
	LinkDescriptor[] projectLinks() {
		return new LinkDescriptor[] {
				...
				linkWithRel("parent").optional().description("Link to parent project"),
				linkWithRel("generations").optional().description("Link to support information"),
				linkWithRel("releases").optional().description("Link to releases information")
		};
	}
